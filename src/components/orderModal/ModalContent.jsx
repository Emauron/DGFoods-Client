...
export default function ModalContent({ product, handleClose }) {
    ...
    const [orderProduct, setOrderProduct] = useState(() => ({
        ...
        totalPrice: getPrecoBRL(), // string formatada
        options: [],               // será populado pelos OrderChoices
    }));

    // Quando trocar de produto, reseta opções e base
    useEffect(() => {
        setOrderProduct({
        id: product.id,
        id_store: product.store_id,
    ...
    // Mantém totalPrice sincronizado com os ajustes
    useEffect(() => {
    ...
    return (
        <div className="group relative flex flex-col rounded-2xl border bg-white shadow-md hover:shadow-lg transition-shadow duration-300 max-h-[80vh] w-full overflow-auto">
            ...
            {/* Opções: emite soma de ajustes e também popula orderProduct.options */}
            <OrderOptions
                product={product}
            ...
            <div className="sticky bottom-0 z-10 border-t border-gray-200/80 bg-white/85 backdrop-blur supports-[backdrop-filter]:bg-white/60">
                ...
                >
                    <FontAwesomeIcon icon={faShoppingCart} /> {addingToCart ? "Adicionando..." : "Adicionar"}
                </button>
                </div>
            </div>
        </div>
