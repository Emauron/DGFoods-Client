...
export default function Header({ id_city }) {
    ...
    return (
        <header
            role="banner"
            className="w-full border-b border-gray-200/70 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 sticky top-0 z-50"
        >
            {/* Torna o container posicionado para permitir o botão absoluto */}
            <div className="mx-auto max-w-7xl px-3 sm:px-4 md:px-6 relative">
                {/* Botão no topo (só mobile), absoluto para não empurrar o layout */}
                <a
                    href="#quero-ser-parceiro"
                    className="sm:hidden absolute right-3 top-2 z-10 inline-flex items-center justify-center rounded-lg bg-gray-900 px-3 py-2 text-xs font-medium text-white shadow-sm active:scale-[.99]"
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 items-center gap-3 py-3 md:py-4 lg:py-5">
                    ...
                    <div
                        className="col-span-1 flex items-center gap-3 min-w-0"
                    >
                        <img
                            src={logoImage}
                            alt="Logomarca"
                            className="h-14 sm:h-16 md:h-14 lg:h-16 w-auto select-none cursor-pointer"
                            draggable={false}
                            onClick={() => navigate("/")}
                        />
